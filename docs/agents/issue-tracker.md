...
- **Frontier query**: read the map's children in map order from the sub-issues
  ...
  the first:
  `gh api repos/<owner>/<repo>/issues/<map>/sub_issues --paginate --jq '.[] | select(.state == "open" and (.assignees | length) == 0 and (.issue_dependencies_summary.blocked_by // 0) == 0) | .number' | head -n 1`.
  No output means the frontier is empty. The `// 0` keeps children whose native
  dependency data is missing; in a repo without native dependencies, drop the
  `head -n 1` and take the first listed child whose `Blocked by` line names no
  open issue. Where sub-issues aren't enabled, walk the map body's task list in
  order instead, dropping tickets that are closed, assigned, or have an open
  issue in their `Blocked by` line.
...
## Labels and hierarchy in this repo

Maps nest through native sub-issues: a map can be a child of another map (for
example, #48 under #46). Beyond the triage labels in
`docs/agents/triage-labels.md`, this repo uses:

- `wayfinder:map`: a map, nested or not.
- `destination:deployment`, `destination:merge`, `destination:decision`: how
  far a map's destination reaches (see below). Every new map carries exactly
  one.
- `wayfinder:research`, `wayfinder:prototype`, `wayfinder:grilling`,
  `wayfinder:task`: the ticket type of a map child that isn't itself a map.
  Each such child carries exactly one.
- `wayfinder:seed`: a loose idea parked for a future map, filed on its own
  rather than as a map child.

### Map destinations

A new map carries execution by default, so its destination is the finished
change rather than only the way to it. Wayfinder treats a map as planning-only
unless its Notes say otherwise, so state the destination in both places: the
label, and a Notes line saying that execution is carried in the map and how
far it goes.

- `destination:deployment` (the default): the map carries execution through
  deployment and post-deployment verification, as #46 does.
- `destination:merge`: the map carries execution through changes merged to
  `main` and stops before deployment.
- `destination:decision`: wayfinder's own default. The map ends when the way
  is clear, and `to-tickets` then projects its route into execution tickets
  elsewhere.

On a `destination:merge` or `destination:deployment` map, each execution child
is a `wayfinder:task`. It also carries `ready-for-agent` once it is fully
specified and an agent can take it without a human; check its native blockers
before claiming. Execution tickets that `to-tickets` projects from a
`destination:decision` map carry `ready-for-agent`, plus `wayfinder:task` when
they become children of a map.

Triage roles apply to issues that arrive as requests or findings.
