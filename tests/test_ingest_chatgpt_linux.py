...
class ChatGPTLinuxIngestTests(unittest.TestCase):
    ...
    def test_update_removes_replaced_package_archive_signatures(self):
        old_package = self._create_package(
            "fixture", "1.0-1", {"usr/bin/fixture": b"old\n"}
        )
        new_package = self._create_package(
            "fixture", "2.0-1", {"usr/bin/fixture": b"new\n"}
        )
        staged_old = self.repo / old_package.name
        staged_old_signature = Path(f"{staged_old}.sig")
        staged_new = self.repo / new_package.name
        stale_new_signature = Path(f"{staged_new}.sig")
        shutil.copy2(old_package, staged_old)
        staged_old_signature.write_bytes(b"old detached signature")
        stale_new_signature.write_bytes(b"stale same-filename signature")
        subprocess.run(
            ["repo-add", str(self.repo / "fixture.db.tar.zst"), str(staged_old)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            check=True,
        )

        package_dir = self.root / "update-fixture"
        fake_bin = self.root / "update-bin"
        package_dir.mkdir()
        fake_bin.mkdir()
        ...
        fake_makepkg.write_text(
            f"#!{sys.executable}\n"
            "import sys\n"
            f"package = {str(new_package)!r}\n"
            "if sys.argv[1:] == ['--packagelist']:\n"
            "    print(package)\n"
            "elif sys.argv[1:] == ['--printsrcinfo']:\n"
            "    print('pkgname = fixture')\n"
            "else:\n"
            "    raise SystemExit(2)\n",
            encoding="utf-8",
        )
        fake_makepkg.chmod(0o755)
        signature_scan_marker = self.root / "signature-scanned"
        real_pacman = _resolve_real_binary("pacman")
        fake_pacman = fake_bin / "pacman"
        fake_pacman.write_text(
            f"#!{sys.executable}\n"
            "import os\n"
            "import sys\n"
            f"marker = {str(signature_scan_marker)!r}\n"
            f"real_pacman = {real_pacman!r}\n"
            "if sys.argv[-1].endswith('.sig'):\n"
            "    open(marker, 'ab').close()\n"
            "    raise SystemExit(88)\n"
            "os.execv(real_pacman, [real_pacman, *sys.argv[1:]])\n",
            encoding="utf-8",
        )
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertFalse(staged_old.exists())
        self.assertFalse(staged_old_signature.exists())
        self.assertTrue(staged_new.is_file())
        self.assertEqual(self._sha256(staged_new), self._sha256(new_package))
        self.assertFalse(stale_new_signature.exists())
        self.assertFalse(signature_scan_marker.exists())
        self.assertFalse(Path(f"{self.repo}.writer.lock").exists())
